 ...
 * */
     ...
     int sine_lod_level) // New parameter for sine LOD
 {
     ...
     if (finalRandSwitch == 1) {
         return portable_rand((int)(active_stream_val_double * FPSR_INFLATION_FACTOR)); // Seed is already large integer
     } else {
  ...
  * @param query_step_size (int) The step size (in original frame units) used for change detection.
  * This defines how far back the 'previous frame' is for has_changed and search operations.
 ...
 FPSR_Output fpsr_sm_get_details(
     ...
     int query_step_size, // New parameter
     int minHold, int maxHold,
     int reseedInterval, int seedInner, int seedOuter, int finalRandSwitch,
     int lod, int max_search_frames)
 {
     ...
     // based on query_step_size
     double prev_scaled_frame_for_lod1_double = (double)(frame - query_step_size) * frame_multiplier; 
     ...
     int step_int = query_step_size; // Use query_step_size for exponential probe step
     ...
     while(low_int <= high_int) {
         ...
         if (_fpsr_sm_base(mid_frame_double, minHold, maxHold, reseedInterval, seedInner, seedOuter, finalRandSwitch) == out.randVal) {
             ...
             // using query_step_size as the step for comparison
             double mid_minus_step_frame_double = (double)(mid_int - query_step_size) * frame_multiplier; 
     ...
     int lod, int max_search_frames)
 {
     ...
     // Forwards search
     int bound_high_int = frame; // operates on original frame space (int)
     step_int = query_step_size; // Use query_step_size for exponential probe step
     while (frame + step_int < frame + max_search_frames) { // All int
         // Scale the probe frame (as double) before passing to base algorithm
         double probe_frame_double = (double)(frame + step_int) * frame_multiplier; // All double
